...
exports.showCharacterDetails = showCharacterDetails;
...
async function handleCharacterCommand(interaction) {
    ...
    const characterId = interaction.options.getString(commands_1.OPTION_NAMES.CHARACTER_ID);
...
async function showCharacterDetails(interaction, uid, characterId) {
    try {
        ...
        const charName = characters_1.characterNames[characterId] || `キャラID: ${characterId}`;
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: 'レベル', value: `${level}`, inline: true }, { name: '命ノ星座', value: `${constellation}`, inline: true });
        // 戦闘ステータス
