...
async function handleRegisterCommand(interaction) {
    ...
    try {
        ...
        // ユーザーデータを保存
        await (0, userData_1.setUserUID)(userId, uid, interaction.user.username);
        const embed = new discord_js_1.EmbedBuilder()
            .setColor(0x00FF00)
            .setTitle('✅ UID登録完了')
            .setDescription(`あなたのUID \`${uid}\` を登録しました！`)
            .addFields({ name: '📌 使い方', value: '今後は `/my-genshin` コマンドでUID入力なしで情報を表示できます。', inline: false }, { name: '🔄 更新', value: 'UIDを変更したい場合は、再度このコマンドを実行してください。', inline: false })
